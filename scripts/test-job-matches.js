...
async function testJobMatches() {
  try {
    ...
    console.log('\nTest 1: Get job matches');
    const matchesResponse = await fetch(`http://localhost:3000/api/jobs/${jobId}/matches?exclude_fields=embedding`);
    ...
    console.log('\nTest 2: Update processed data');
    const updateResponse = await fetch(`http://localhost:3000/api/jobs/${jobId}/matches?update_stats=true&size=1`);
