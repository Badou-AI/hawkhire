...
import { type Job, type JobType, type JobStatus } from '@/types/job'
    ...
    async function fetchJobs() {
      try {
        ...
        setJobs(data.map((job: DatabaseJob) => ({
          ...
          status: (job.status || 'published') as JobStatus,
          remote: job.remote,
          skills: job.skills || [],
          ...
          organization: job.organization ? {
            ...
            logo_url: job.organization.logo_url || '/placeholder-logo.png',
            slug: ''
          } : undefined
        })))
      } catch (error) {
            ...
            <Link href={`/jobs/${job.id}`} className="block h-full">
              <Card className="flex h-full flex-col p-6 transition-colors hover:border-primary">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-medium">{job.organization?.name || ''}</h3>
                ...
                <div className="mt-4 mt-auto">
                  <Button variant="link" className="h-auto p-0 text-primary">
                    {job.remote ? 'Remote' : 'On-site'} • {job.salary_min && job.salary_max ? `$${job.salary_min/1000}k - $${job.salary_max/1000}k` : 'Competitive'}
                  </Button>
                </div>
              </Card>
            </Link>
